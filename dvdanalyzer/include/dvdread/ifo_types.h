...
typedef struct {
  ...
  uint32_t pgc_start_byte;//开始
  pgc_t *pgc;//节目链信息
} ATTRIBUTE_PACKED pgci_srp_t;//节目链信息搜索指针
...
typedef struct {
  ...
  uint16_t zero_1;
  uint32_t last_byte;
  pgci_srp_t *pgci_srp;//搜索指针
  int      ref_count;//搜索指针数
} ATTRIBUTE_PACKED pgcit_t;//VTS节目链信息表
...
typedef struct {
  ...
  uint8_t  title_set_nr;//哪个标题（标题1，还是标题2）
  uint8_t  vts_ttn;//ttn
 ...
 */
typedef struct {
  uint16_t nr_of_srpts;//个数,title总数量
  uint16_t zero_1;
  uint32_t last_byte;
  title_info_t *title;//标题信息结构
...
} ATTRIBUTE_PACKED vts_ptt_srpt_t;//VTS章节搜索指针表
