...
////extern int UDFReadBlocksRaw( dvd_reader_t *device, uint32_t lb_number, size_t block_count, unsigned char *data,  int encrypted );
...
int iso_read_volume_descriptor(dvd_reader_t *aDevice, struct iso_volume_desc_t *aVolumeDesc)
    {
...
void iso_path_table_free(struct iso_path_table_record_t *list)
    {
...
int iso_read_path_table(dvd_reader_t *aDevice, struct iso_volume_desc_t *aVolumeDesc, struct iso_path_table_record_t **aRecords)
    {
    uint8_t *content, *ptr;
    ...
    for (ptr = content; sector <= last_sector; ++sector, ptr += ISO_SECTOR_SIZE)
        ...
        if (InternalUDFReadBlocksRaw(aDevice, sector, 1, ptr, 0) <= 0)
            {
            fprintf(stderr, "InternalUDFReadBlocksRaw   return FAILURE   hanzc\n " );
            free(content);
            return FAILURE;
            }
        }
    for (ptr = content; ptr < content + aVolumeDesc->mPathTblLen;)
        ...
        ///fprintf (stdout, "MY===Name Length : %u\n", (int) current->mNameLen );
...
uint32_t iso_search_directory(dvd_reader_t *aDevice, uint32_t aDirectorySector, char *aFilename, uint32_t *aFileSize)
    {
    ...
    *aFileSize = 0;
    ...
    for (position = 0; position < directorySize; position += recordSize)
        ...
        if (position >= ISO_SECTOR_SIZE || content[position] == 0)
            ...
            if (InternalUDFReadBlocksRaw(aDevice, ++aDirectorySector, 1, content, 0) <= 0)
                {
                fprintf(stderr, "InternalUDFReadBlocksRaw  read error hanzc\n " );
                return 0;
                }
            position = 0;
...
#endif /* ISO_DEBUG */
        if (!IS_DIRECTORY(flags) && (filenameLen == identifierSize || (filenameLen < identifierSize && 
...
uint32_t ISOFindFile(dvd_reader_t *aDevice, char *aFilename, uint32_t *aFileSize)
    ...
    *aFileSize = 0;
    ...
    if (iso_read_volume_descriptor(aDevice, &volumeDesc) != SUCCESS)
        {
         fprintf(stderr, "iso_read_volume_descriptor   return 0  hanzc\n " );
        return 0;
        }
#ifdef ISO_DEBUG
    fprintf(stderr,
    ...
    while (last != '\0' && *rest != '\0' && current != NULL)
        ...
        last = *rest;
        *rest++ = '\0';
        //fprintf(stderr, " ===9660 FindFile ====  rest=%s \n ",rest );  //////  ===hanzhicai ====  rest=VIDEO_TS.IFO
    ...
    if (current == NULL)
        ...
        free(path);
        fprintf(stderr, "iso_read_volume_descriptor   current == NULL  hanzc\n " );
        return 0;
        }
