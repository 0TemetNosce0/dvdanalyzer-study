...
static int SetUDF_ISO9600 =1 ;    /////// UDF =1 ; ISO9600 = 2 ;
static int Get_DVDISCSSEncrypted =0 ;   //// 0 表示未加密,  1 表示加密 
 ...
 void Set_DVDRead_UDFISO9600 (int UDF_ISO)  /////   /////// UDF =1 ; ISO9600 = 2 ; Try UDF then ISO =0 ;
{
 ...
 int Get_DVDDiscisEncrypted ()     ///// 得到是否找到key
{
	return Get_DVDISCSSEncrypted ;
}

uint32_t DVDFindFile( dvd_reader_t *device, char *filename, uint32_t *size  , int isVOB)
{
    uint32_t result = 0;
    const char* without_video_ts_folder_filename = NULL;
    if( !strncasecmp( "/VIDEO_TS/", filename, 10 ) ) {
        without_video_ts_folder_filename = &(filename[ 9 ]);
    }

    /////////////return UDFFindFile(device, filename, size);
    if ( SetUDF_ISO9600 == 0 )
    {
        // 先用UDF方式找/VIDEO_TS目录下的文件

        result = UDFFindFile(device, filename, size);

        // 没找到，用ISO方式找/VIDEO_TS目录下的文件
        if (result <=0) {
            result = ISOFindFile(device, filename, size);
        }

        // 没找到，用UDF方式找根目录的文件
        if (result <= 0) {
            result = UDFFindFile(device, without_video_ts_folder_filename, size);
        }

        // 没找到，用ISO方式找根目录的文件
        if (result <= 0) {
            result = ISOFindFile(device, without_video_ts_folder_filename, size);
        }
    }
    else if ( SetUDF_ISO9600 == 1 )  //// UDF
    {
        //fprintf(stderr, "Set DVD AS UDF Disc \n" );
        result = UDFFindFile(device, filename, size);
        // 没找到，用UDF方式找根目录的文件
        if (result <= 0) {
            result = UDFFindFile(device, without_video_ts_folder_filename, size);
    ...
    else if ( SetUDF_ISO9600 == 2 )    ///// ISO9600
    {
        //fprintf(stderr, "Set DVD AS ISO9600 Disc \n" );
        result = ISOFindFile(device, filename, size);
        // 没找到，用ISO方式找根目录的文件
        if (result <= 0) {
            result = ISOFindFile(device, without_video_ts_folder_filename, size);
...
static int initAllCSSKeys( dvd_reader_t *dvd )
{
  ...
  for( title = 0; title < 100; title++ ) {
    ...
    //start = UDFFindFile( dvd, filename, &len );    //// hanzhicai 20161023
    	start = DVDFindFile( dvd, filename, &len , 1 );
...
/**
 * Open a DVD image or block device file or use stream_cb functions.    //////DVDOpenImageFile( NULL, stream, stream_cb, have_css );
 */
                                    ...
                                    dvd_reader_stream_cb *stream_cb )
{
  ...
  /* Try to open DVD using stream_cb functions */   ////////DVDOpenCommon( ppath, NULL, NULL );
 ...
 * Open an unencrypted file on a DVD image file.
 ...
 ////  这个函数其实只用于读取 IFO 或者 BUP ，不读 VOB 
 */    /// this function read file contents ，from DVDOpenFile function  ;  
                                   ...
                                   int do_cache )
{
 ...
 //// start = UDFFindFile( dvd, filename, &len );     ///// hanzhicai 20161023
  start = DVDFindFile( dvd, filename, &len , 0 );
  ...
  if( do_cache && len <  640 * DVD_VIDEO_LB_LEN ) {
    int ret;
    ...
    //return dvd_file;     ///// hanzhicai 改动，为了下面有机会返回 NULL 
  } 	
...
static dvd_file_t *DVDOpenVOBUDF( dvd_reader_t *dvd, int title, int menu )
{
  ...
  if( !menu ) {
    ...
    for( cur = 2; cur < 10; cur++ ) {
      ...
      ////if( !UDFFindFile( dvd, filename, &len ) ) break;    ///// hanzhicai 20161023
      if( !DVDFindFile( dvd, filename, &len , 1 ) ) break;  
      dvd_file->filesize += len / DVD_VIDEO_LB_LEN;
    }
  }
...
/*
	现在问题找到了, ISO9660 情况下，发现 :
	record size:   48
file position: 387135
file size:     33581056
directory:     0
name size:     14
name:          "VTS_01_0.IFO;1"
文件太大， 33 M ， 明显是个假文件， 但是对应的 BUP 文件是正确的
*/
//dvd_reader_t,
//dvd_read_domain_t:vob，ifo。bup
//返回文件信息结构dvd_file_t
//titlenum:0/VIDEO_TS/VIDEO_TS.IFO,否则/VIDEO_TS/VTS_XX_0.IFO
                              ...
                              int menu, dvd_stat_t *statbuf )
{
 ...
 // if( !UDFFindFile( dvd, filename, &size ) )       //// hanzhicai 原始 patch 没有, 自己添加, 有可能错误  20161023 
 		if( !DVDFindFile( dvd, filename, &size ,1 ) )
    return -1;
                 ...
                 dvd_read_domain_t domain, dvd_stat_t *statbuf )
{
  ...
  case DVD_READ_TITLE_VOBS:
    if( titlenum == 0 )
      return -1;
		...
		fprintf( stderr, "dvd->isImageFile =%d  \n" ,dvd->isImageFile);
    if( dvd->isImageFile )
  ...
  if( dvd->isImageFile ) {
    //// if( UDFFindFile( dvd, filename, &size ) ) {    //// hanzhicai 原始 patch 没有, 自己添加, 有可能错误  20161023 
    if( DVDFindFile( dvd, filename, &size, 0 ) ) {
      statbuf->size = size;
      statbuf->nr_parts = 1;
...
fprintf( stderr, "libdvdread: DVDDiscId =====================>>>>>>>>>>>>>  \n" );
  /* Check arguments. */
