...
int ifoRead_TT_SRPT(ifo_handle_t *ifofile) {
  ...
  //如果数据错误，强制有一个title
  if (tt_srpt->nr_of_srpts <= 0) {
    tt_srpt->nr_of_srpts = 1;
  }

  //dxy 有些DVD的last_byte为0，
  //如果title的数量大于零，就用这个长度反推表长度。
  //如果title数量不大于0，返回错误。
  if (tt_srpt->last_byte > TT_SRPT_SIZE) {
      info_length = tt_srpt->last_byte + 1 - TT_SRPT_SIZE;
  }
  else if (tt_srpt->nr_of_srpts > 0){
...
int ifoRead_VTS_PTT_SRPT(ifo_handle_t *ifofile) {
  ...
  for(i = 0; i < vts_ptt_srpt->nr_of_srpts; i++) {
    ...
    for(j = 0; j < vts_ptt_srpt->title[i].nr_of_ptts; j++) {
      ...
      //////  hanzhicai 20161105  有些不正规的碟子 以下代码会导致错误，其实碟子是可以正常播放  
      /////  这是重点修改的地方
      /* if (//vts_ptt_srpt->title[i].ptt[j].pgcn == 0 ||      
         ...
         fprintf(stderr, "vts_ptt_srpt->title[i].ptt[j].pgcn =%d  ; vts_ptt_srpt->title[i].ptt[j].pgn =%d  .\n", vts_ptt_srpt->title[i].ptt[j].pgcn , vts_ptt_srpt->title[i].ptt[j].pgn  );
        return 0;
